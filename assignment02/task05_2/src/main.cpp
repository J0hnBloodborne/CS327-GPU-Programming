...
{
    FILE* csv = nullptr;
    if (fopen_s(&csv, "../../data/results3.csv", "w") != 0 || csv == nullptr) {
        std::cerr << "Failed to open data/results3.csv for writing\n";
        return 1;
    }
    std::fprintf(csv, "rows,cols,gpu_ms,tiled_ms\n");
    ...
    for (int i = 0; i < 10; i++) {
        ...
        printf("Input %d: GPU Time: %.3f ms, Tiled Time: %.3f ms\n", i, gpu_ms, tiled_ms);
        std::fprintf(csv, "%d,%d,%.6f,%.6f\n", matsM[i].rows, matsM[i].cols, gpu_ms, tiled_ms);
    }
}
