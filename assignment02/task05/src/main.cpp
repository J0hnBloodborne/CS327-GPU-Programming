...
{
    FILE* csv = nullptr;
    if (fopen_s(&csv, "../../data/results2.csv", "w") != 0 || csv == nullptr) {
        std::cerr << "Failed to open data/results.csv for writing\n";
        return 1;
    }
    std::fprintf(csv, "rows,cols,cpu_ms,gpu_ms,tiled_ms\n");
    ...
    for (int i = 0; i < 10; i++) {
        ...
        double tiled_ms = time_function([&] { mulMatGPUTiled(matsM[i], matsN[i]); });
        printf("Input %d: CPU Time: %.3f ms, GPU Time: %.3f ms, Tiled Time: %.3f ms\n", i, cpu_ms, gpu_ms, tiled_ms);
        std::fprintf(csv, "%d,%d,%.6f,%.6f,%.6f\n", matsM[i].rows, matsM[i].cols, cpu_ms, gpu_ms, tiled_ms);
    }
}
